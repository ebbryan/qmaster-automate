...
// import { PaymentPrioContext } from "@/pages/counter-personnel/application";
//API
...
export const PaymentPriority: FunctionComponent<ConsumerProps> = () => {
  ...
  const { consumerTransaction, setConsumerTransaction } =
    React.useContext(PaymentPrioContext);
  ...
  const getConsumerDataRegular = useCallback(async () => {
    try {
      ...
      const resultRegular = await axios.get(
        `${API_Consumer_Transaction}/transactionstatus?consumerPriority=2&transactionStatus=1&transactionType=3`,
        {
          headers: {
