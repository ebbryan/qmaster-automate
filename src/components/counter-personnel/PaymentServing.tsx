...
import { PaymentPrioContext } from "@/pages/counter-personnel/payment";
import PaymentPriority from "./PaymentPriority";
import { PaymentDrop } from "./PaymentDrop";
import RegularPayment from "./RegularPayment";
import PriorityPayment from "./PriorityPayment";
...
export type PaymentTableContextState = {
...
export const PaymentTableContext = React.createContext(
  {} as PaymentTableContextState
);

export const ServingPayment: FunctionComponent<ServingProps> = ({}) => {
  const [consumerData, setconsumerData] = useState<
    Array<ConsumerTransactionType>
  >([]);
  const [transferDataWelfare, setTransferWelfare] = useState<
    Array<TransferredFrom>
  >([]);
  const [transferDataApp, setTransferApp] = useState<Array<TransferredFrom>>(
    []
  );
  const now = new Date();
  ...
  const speak = async (consumer: ConsumerTransactionType | undefined) => {
    if (!speaking && consumer) {
      const lastText = "Please proceed to Payment.";
  ...
  const fetchNowServing = useCallback(async () => {
    try {
      ...
      const result = await axios.get(`${API_NowServing_Pay}`, {
        headers: {
    ...
    const getConsumer = async () => {
      try {
        const consumer = await axios.get(
          `${API_Consumer_Transaction}/transactionstatus?transactionStatus=2&transactionType=3&counterPersonnel=${subID}`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`, // include the access token in the Authorization header
            },
    ...
    const getTransferFromWelfare = async () => {
      ...
      try {
        const transferResult = await axios.get(
          `${API_Transfer}/transfer-from?transactionFrom=CUSTOMER WELFARE&transactionType=3`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`, // include the access token in the Authorization header
            },
        ...
        setTransferWelfare(transferResult.data);
        // console.log(transaction);
      } catch (error) {}
    };
    const getTransferFromApplication = async () => {
      // const transaction = localStorage.getItem("transactionType");
      try {
        const transferResult = await axios.get(
          `${API_Transfer}/transfer-from?transactionFrom=APPLICATION&transactionType=3`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`, // include the access token in the Authorization header
            },
        ...
        setTransferApp(transferResult.data);
        // console.log(transaction);
      } catch (error) {}
    };
    fetchNowServing();
    getTransferFromWelfare();
    getTransferFromApplication();
    ...
    <div>
      <div className="grid grid-cols-3">
        <div>
          ...
          >
            <PaymentRegular consumerTransactionType={[]} />
        ...
        <div>
          ...
          <PriorityPayment
            ...
            speak={speak}
          />
          {/* <PaymentPrioContext.Provider
            value={{
              consumerTransaction: selectedConsumer || undefined, // diri
              setConsumerTransaction: setSelectedConsumer,
            }}
          >
            <PaymentPriority consumerTransactionType={[]} />
        ...
        <div>
          <h1 className="px-9 py-2 mx-2 bg-red-800 flex flex-col justify-center font-montserrat font-bold uppercase text-lg text-white">
            Unattended
          </h1>
          <DropPayment
            consumerData={consumerData}
          ...
          >
            <PaymentDrop consumerTransactionType={[]} />
      ...
      <div className="grid grid-cols-2">
        <div>
          ...
          <div className="px-9 py-2 bg-blue-900 rounded-lg mt-3 ml-5 flex flex-col justify-center my-2">
        ...
        <div className="grid grid-cols-2">
          <div className="">
            ...
            <div className="px-7 mx-2 rounded-md shadow-md">
              ...
              {transferDataApp.map((transactionTransfer) => {
                return (
                  <h1
                    key={transactionTransfer.id}
                    className="text-lg text-black capitalize"
