...
import RegularWelfare from "./RegularWelfare";
import PriorityWelfare from "./PriorityWelfare";
import DropWelfare from "./DropWelfare";

type ServingProps = {
  consumerTransactionType: ConsumerTransactionType[];
};

export type WelfareTableContextState = {
  consumerTransaction: ConsumerTransactionType | undefined;
  setConsumerTransaction: (transaction: ConsumerTransactionType) => void;
};
export const WelfareTableContext = React.createContext(
  {} as WelfareTableContextState
);

export const ServingWelfare: FunctionComponent<ServingProps> = ({}) => {
  ...
  useEffect(() => {
    ...
    const getSubTransactionData = async () => {
      try {
        const subtransaction = await axios.get(
          `${API_Sub_Transact}/transaction-type?transactionType=2`,
          {
            headers: {
        ...
        console.log(subtransaction.data);
        reset();
      } catch (error) {}
    };
    const getConsumer = async () => {
      try {
        const consumer = await axios.get(
          `${API_Consumer_Transaction}/transactionstatus?transactionStatus=2&transactionType=2&counterPersonnel=${subID}`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`, // include the access token in the Authorization header
            },
    ...
    const getTransferFromPayment = async () => {
      ...
      try {
        const transferResult = await axios.get(
          `${API_Transfer}/transfer-from?transactionFrom=PAYMENT&transactionType=2`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`, // include the access token in the Authorization header
            },
    ...
    const getTransferFromApplication = async () => {
      ...
      try {
        const transferResult = await axios.get(
          `${API_Transfer}/transfer-from?transactionFrom=APPLICATION&transactionType=2`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`, // include the access token in the Authorization header
            },
  ...
  const handleComplete = async (consumer: ConsumerTransactionType) => {
    try {
      ...
      setconsumerData((prevConsumerData) =>
        prevConsumerData.filter((item) => item.id !== consumer.id)
      );
    ...
    <div>
      <div className="grid grid-cols-3">
        <div>
          ...
          {/* <WelfareTable consumerTransactionType={[]} /> */}
          {/* </WelfareTableContext.Provider> */}
        </div>
        <div>
          <h1 className="px-9 py-2 mx-2 bg-blue-800  flex flex-col justify-center font-montserrat font-bold uppercase text-lg text-white">
        ...
        <div>
          <button
            ...
            onClick={(e) => {
              e.preventDefault();
              handleClick();
              if (selectedConsumer === null && consumerData.length === 1) {
        ...
        <div>
          <button
            ...
            onClick={(e) => {
              e.preventDefault();
              handleClick();
              if (selectedConsumer && sub) {
                handlePatchServe(selectedConsumer);
                handleComplete(consumerData[0]);
                speak(selectedConsumer);
              }
            }}
            disabled={!selectedConsumer || !sub}
            className={`bg-white text-gray-800 px-3 py-2 border border-blue-900 flex items-center rounded-md hover:shadow-lg hover:bg-blue-800 hover:text-white transition-all font-poppins font-medium text-xl container ${
              selectedConsumer && !sub ? "opacity-50 cursor-not-allowed" : ""
            }`}
          >
            <HiChevronDoubleRight className="text-5xl mr-2" /> Next
          </button>
