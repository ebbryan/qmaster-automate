  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.get(
        `${dropConsumer}/transactionstatus?transactionType=2&transactionStatus=4`,
        {
          headers: {
    ...
    return visibleData.map((row, index) => {
      ...
      return (
        <tr
          key={index}
          className={`${
            isSelected ? "bg-blue-800 rounded-lg  text-white" : ""
          } mx-1 gap-1`}
          onClick={() => handleRowClick(index)} // Attach the onClick event
        >
          <td className="uppercase text-xl  flex flex-grow py-2 mx-2 px-4">
            {row.name}
          </td>
        </tr>
      );
