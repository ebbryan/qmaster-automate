...
export const AppRegular: FunctionComponent<ConsumerProps> = () => {
  ...
  const { consumerTransaction, setConsumerTransaction } =
    React.useContext(AppTableContext);
  ...
  const getConsumerDataRegular = useCallback(async () => {
    try {
      ...
      const resultRegular = await axios.get(
        `${API_Consumer_Transaction}/transactionstatus?consumerPriority=1&transactionStatus=1&transactionType=1`,
        {
          headers: {
