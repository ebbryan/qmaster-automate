...
export const WelfarePriority: FunctionComponent<ConsumerProps> = () => {
  ...
  const { consumerTransaction, setConsumerTransaction } =
    React.useContext(WelfarePrioContext);
  ...
  const getConsumerDataPriority = useCallback(async () => {
    try {
      ...
      const resultPriority = await axios.get(
        `${API_Consumer_Transaction}/transactionstatus?consumerPriority=2&transactionStatus=1&transactionType=2`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`, // include the access token in the Authorization header
          },
...
export default WelfarePriority;
